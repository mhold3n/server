...
def run_tests(test_type="all", verbose=True, coverage=False):
    ...
    if coverage:
        cmd.extend(
            [
                "--cov=services/prompt_middleware",
                "--cov-report=html",
                "--cov-report=term",
            ]
        )
    ...
    if test_type == "all":
        cmd.extend(
            ["tests/test_prompt_middleware.py", "tests/test_object_inference.py"]
        )
    elif test_type == "unit":
...
def main():
    ...
    # Run tests
    success = run_tests(
        test_type=args.type, verbose=not args.no_verbose, coverage=args.coverage
    )
