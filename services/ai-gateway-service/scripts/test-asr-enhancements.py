...
class ASRTester:
    ...
    def check_asr_health(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if model_loaded:
                    self.print_status(
                        "✅ ASR service is healthy and model is loaded", "success"
                    )
                    ...
                    self.print_status(
                        "❌ ASR service is running but model not loaded", "error"
                    )
                    return False
            else:
                self.print_status(
                    f"❌ ASR service health check failed: {response.status_code}",
                    "error",
                )
                return False
    ...
    def test_base64_transcription(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/transcribe", json=payload, timeout=60
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"   Technical segments: {len(result.get('technical_segments', []))}"
                )
                ...
                self.print_status(
                    f"❌ Base64 transcription failed: {response.status_code}", "error"
                )
        ...
        for test_case in test_urls:
            ...
            try:
                ...
                if response.status_code == 200:
                    result = response.json()
                    self.print_status(
                        f"✅ URL transcription completed: {description}", "success"
                    )
                    print(f"   Response time: {response_time:.2f}s")
                    print(f"   Transcript: '{result.get('transcript', 'N/A')}'")
                    ...
                    self.print_status(
                        f"❌ URL transcription failed: {description}", "error"
                    )
                ...
                results[description] = {"success": False, "url": url, "error": str(e)}
    ...
    def test_error_handling(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            ...
            try:
                response = requests.post(
                    f"{self.base_url}/transcribe", json=payload, timeout=30
                )
                ...
                if response.status_code == expected_status:
                    ...
                    self.print_status(
                        f"⚠️  Unexpected status for {name}: {response.status_code} (expected {expected_status})",
                        "warning",
                    )
                    results[name] = {
                        "success": False,
        ...
        for i, test_case in enumerate(test_texts):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   Is technical: {is_technical} (expected: {expected_technical})"
                    )
                    ...
                    self.print_status(
                        f"❌ Technical analysis failed: {response.status_code}", "error"
                    )
                    results[f"test_{i+1}"] = {
                        "success": False,
                        "status": response.status_code,
    ...
    def print_summary(self):
        ...
        for test_name, result in self.test_results.items():
            ...
            if test_name == "base64_transcription":
                ...
                success = (
                    any(test.get("success", False) for test in result.values())
                    if isinstance(result, dict)
                    else False
                )
            elif test_name == "error_handling":
                success = (
                    all(test.get("success", False) for test in result.values())
                    if isinstance(result, dict)
                    else False
                )
        ...
        if passed_tests == total_tests:
            self.print_status(
                "🎉 ALL TESTS PASSED! ASR enhancements working correctly.", "success"
            )
        elif passed_tests > 0:
            self.print_status(
                f"⚠️  PARTIAL SUCCESS: {passed_tests}/{total_tests} test suites passed.",
                "warning",
            )
        else:
            self.print_status(
                "❌ ALL TESTS FAILED: ASR enhancements need attention.", "error"
            )
        ...
        print(
            f"{Colors.YELLOW}Base64 and error handling tests are more reliable indicators.{Colors.NC}"
        )
    ...
    async def run_all_tests(self):
        ...
        if not self.check_asr_health():
            self.print_status(
                "❌ ASR service not available. Tests cannot proceed.", "error"
            )
            return
...
async def main():
    ...
    parser.add_argument(
        "--url", default="http://localhost:8084", help="ASR service URL"
    )
    parser.add_argument(
        "--test",
        choices=["base64", "url", "error", "technical", "all"],
        default="all",
