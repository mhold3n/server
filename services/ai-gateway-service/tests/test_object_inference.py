...
class TestObjectInference:
    ...
    async def test_object_inference_accuracy(self, ambiguous_coding_scenarios):
        ...
        for scenario in ambiguous_coding_scenarios:
            ...
            context = PromptContext(
                ...
                temporal_context={"urgency": scenario["context_clues"]["urgency"]},
            )
            ...
            candidate = (
                max(transformed_texts, key=lambda t: len(t))
            ...
            if self._is_object_inference_correct(
                inferred_object, scenario["true_object"]
            ):
                correct_inferences += 1
                print(
                    f"✅ Correctly inferred '{scenario['true_object']}' from '{scenario['prompt']}'"
                )
            else:
                print(
                    f"❌ Failed to infer '{scenario['true_object']}' from '{scenario['prompt']}'"
                )
    ...
    def _infer_object_from_results(self, results: List, scenario: Dict) -> str:
        ...
        for result in results:
            ...
            if "emotional_undertones" in subtext:
                for emotion, score in subtext["emotional_undertones"].items():
                    all_emotional_undertones[emotion] = (
                        all_emotional_undertones.get(emotion, 0) + score
                    )
        ...
        if total_emotional_score > 0:
            all_emotional_undertones = {
                k: v / total_emotional_score
                for k, v in all_emotional_undertones.items()
            }

        # Calculate average urgency and domain specificity
        avg_urgency = total_urgency / len(results) if results else 0
        avg_domain_specificity = (
            total_domain_specificity / len(results) if results else 0
        )
    ...
    def _classify_object_from_patterns(
        self, emotions: Dict, urgency: float, domain_spec: float, scenario: Dict
    ) -> str:
        ...
        if (
            urgency > 0.6
            and emotions.get("anger", 0) > 0.2
            and "optimize" in scenario["context_clues"]["expected_actions"]
        ):
        ...
        if (
            urgency < 0.4
            and emotions.get("sadness", 0) > 0.2
            and "explain" in scenario["context_clues"]["expected_actions"]
        ):
        ...
        if (
            0.4 <= urgency <= 0.6
            and emotions.get("disgust", 0) > 0.2
            and "refactor" in scenario["context_clues"]["expected_actions"]
        ):
        ...
        if (
            urgency < 0.4
            and emotions.get("fear", 0) > 0.2
            and "review" in scenario["context_clues"]["expected_actions"]
        ):
            return "code_review"
        ...
        if (
            urgency > 0.6
            and emotions.get("surprise", 0) > 0.3
            and "debug" in scenario["context_clues"]["expected_actions"]
        ):
            return "logic_error"
        ...
        if (
            urgency > 0.7
            and emotions.get("anger", 0) > 0.4
            and "timeout" in scenario["context_clues"]["expected_actions"]
        ):
        ...
        if (
            0.4 <= urgency <= 0.6
            and emotions.get("fear", 0) > 0.2
            and "investigate" in scenario["context_clues"]["expected_actions"]
        ):
    ...
    def test_voice_context_object_inference(self, ambiguous_coding_scenarios):
        ...
        for scenario in ambiguous_coding_scenarios:
            ...
            if inferred_object == scenario["true_object"]:
                correct_inferences += 1
                print(
                    f"✅ Voice inference correct: '{scenario['true_object']}' from voice context"
                )
            else:
                print(
                    f"❌ Voice inference failed: expected '{scenario['true_object']}', got '{inferred_object}'"
                )
    ...
    def _infer_object_from_voice(self, subtext, scenario: Dict) -> str:
        ...
        if (
            subtext.urgency_level > 0.7
            and subtext.emotional_state.get("anger", 0) > 0.3
        ):
        ...
        if (
            subtext.urgency_level > 0.6
            and subtext.emotional_state.get("anger", 0) > 0.2
        ):
            return "performance_optimization"

        # Low urgency + confusion = explanation needed
        if (
            subtext.urgency_level < 0.4
            and subtext.emotional_state.get("sadness", 0) > 0.2
        ):
            return "code_explanation"

        # Medium urgency + disappointment = refactoring
        if (
            0.4 <= subtext.urgency_level <= 0.6
            and subtext.emotional_state.get("disgust", 0) > 0.2
        ):
            return "code_refactoring"
        ...
        if (
            subtext.urgency_level > 0.6
            and subtext.emotional_state.get("surprise", 0) > 0.3
        ):
            return "logic_error"
        ...
        if (
            0.4 <= subtext.urgency_level <= 0.6
            and subtext.emotional_state.get("fear", 0) > 0.2
        ):
            return "suspicious_behavior"

        return "unknown"

    def test_geometric_transformation_object_revelation(
        self, ambiguous_coding_scenarios
    ):
        ...
        for scenario in ambiguous_coding_scenarios:
            ...
            transformations = {
                "mirror": transformer.apply_hyperbolic_transformation(
                    embeddings, config
                ),
            ...
            if object_revealed:
                ...
                print(
                    f"✅ Geometric transformation revealed object for: '{scenario['prompt']}'"
                )
            else:
                print(
                    f"❌ Geometric transformation failed to reveal object for: '{scenario['prompt']}'"
                )
    ...
    def _analyze_transformation_revelation(
        self, transformations: Dict, scenario: Dict
    ) -> bool:
        """Analyze if transformations reveal object characteristics"""
        ...
        avg_variance = (
            np.mean(transformation_variance) if transformation_variance else 0
        )
    ...
    def test_ensemble_object_inference(self, ambiguous_coding_scenarios):
        ...
        for scenario in ambiguous_coding_scenarios:
            ...
            if ensemble_inference == scenario["true_object"]:
                ...
                print(
                    f"❌ Ensemble inference failed: expected '{scenario['true_object']}', got '{ensemble_inference}'"
                )
    ...
    def _ensemble_decision(
        self, voice_inf: str, text_inf: str, context_inf: str, scenario: Dict
    ) -> str:
...
class TestAIIntegration:
    ...
    def test_ai_model_object_guessing(self):
        ...
        for test_case in test_cases:
            ...
            if ai_understood_object == test_case["expected_object"]:
                correct_guesses += 1
                print(
                    f"✅ AI correctly guessed object: '{test_case['expected_object']}'"
                )
            else:
                print(
                    f"❌ AI failed to guess object: expected '{test_case['expected_object']}', got '{ai_understood_object}'"
                )
    ...
    def _analyze_ai_response(self, response: str) -> str:
        ...
        if any(
            ...
            for word in ["debug", "bug", "problem", "issue", "error"]
        ):
            return "bug_in_code"
        elif any(
            word in response_lower
            for word in ["optimize", "performance", "faster", "improve"]
        ):
            return "performance_optimization"
        elif any(
            word in response_lower
            for word in ["explain", "document", "clarify", "understand"]
        ):
            return "code_explanation"
        elif any(
            word in response_lower
            for word in ["refactor", "clean", "organize", "structure"]
        ):
            return "code_refactoring"
        elif any(
            word in response_lower
            for word in ["review", "check", "analyze", "validate"]
        ):
            return "code_review"
        elif any(
            word in response_lower for word in ["test", "verify", "logic", "algorithm"]
        ):
            return "logic_error"
            ...
            word in response_lower
            for word in ["investigate", "trace", "analyze", "suspicious"]
        ):
            return "suspicious_behavior"
