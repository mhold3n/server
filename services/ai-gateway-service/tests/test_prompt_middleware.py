...
class TestCodingAmbiguityInference:
    ...
    @pytest.mark.asyncio
    async def test_ambiguous_prompt_inference(
        self, ambiguous_coding_prompts, voice_contexts
    ):
        ...
        for test_case in ambiguous_coding_prompts:
            ...
            for result in results:
                ...
                assert (
                    subtext["domain_specificity"] > 0
                ), f"Low domain specificity for: {test_case['prompt']}"
    ...
    def test_voice_context_inference(self, voice_contexts):
        ...
        assert (
            subtext.emotional_state.get("anger", 0) > 0.2
        ), "Should detect frustration/anger"
    ...
    def test_geometric_transformation_effectiveness(self):
        ...
        mirror_result = transformer.apply_hyperbolic_transformation(
            coding_embeddings, config
        )
        quantum_result = transformer.apply_quantum_transformation(
            coding_embeddings, config
        )
        fractal_result = transformer.apply_fractal_transformation(
            coding_embeddings, config
        )

        # Verify transformations produce different results
        assert not np.array_equal(
            coding_embeddings, mirror_result
        ), "Mirror transformation should change embeddings"
        assert not np.array_equal(
            coding_embeddings, quantum_result
        ), "Quantum transformation should change embeddings"
        assert not np.array_equal(
            coding_embeddings, fractal_result
        ), "Fractal transformation should change embeddings"
        ...
        assert (
            abs(np.linalg.norm(mirror_result) - original_norm) < original_norm * 2.0
    ...
    def test_code_review_ambiguity(self):
        ...
        for prompt, expected_intent in ambiguous_prompts:
            context = PromptContext(
                ...
                domain_context={
                    "domain": "coding",
                    "keywords": ["review", "check", "feedback"],
                },
            )

            assert context.text == prompt
            assert (
                context.voice_features["valence"] > 0
            )  # Indicates seeking help, not reporting problem
...
class TestTransformationValidation:
    ...
    def test_mirror_transformation_preserves_meaning(self):
        ...
        similarity = 1 - np.linalg.norm(
            embeddings - mirrored_embeddings
        ) / np.linalg.norm(embeddings)
        assert (
            similarity > 0.3
        ), "Mirror transformation should preserve some semantic similarity"
    ...
    def test_4d_projection_temporal_context(self):
        ...
        projection_urgent = transformer.dimensional_4d_projection(
            embeddings, time_dimension=0.9
        )
        projection_casual = transformer.dimensional_4d_projection(
            embeddings, time_dimension=0.1
        )
...
class TestPerformance:
    ...
    def test_transformation_speed(self):
        ...
        assert (
            hyperbolic_time < 1.0
        ), f"Hyperbolic transformation too slow: {hyperbolic_time}s"
