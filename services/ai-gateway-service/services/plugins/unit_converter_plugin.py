...
UNIT_CONVERSIONS = {
    ...
    "amount": {"mol": 1.0, "mmol": 0.001, "μmol": 1e-6, "nmol": 1e-9, "kmol": 1000.0},
}
...
class UnitConverterPlugin:
    ...
    def get_available_tools(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "returns": {
                    "type": "object",
                    "description": "Conversion result with metadata",
                },
            },
        ]
    ...
    def convert_temperature(self, value: float, from_unit: str, to_unit: str) -> float:
        ...
        if from_unit not in temp_units or to_unit not in temp_units:
            raise ValueError(
                f"Unsupported temperature unit. Supported: {list(temp_units.keys())}"
            )
    ...
    def convert_units(
        ...
        if unit_type is None:
            raise ValueError(
                f"Cannot convert between {from_unit} and {to_unit} - incompatible unit types"
            )
    ...
    def _convert_simple_unit(
        ...
        if from_unit not in units or to_unit not in units:
            raise ValueError(
                f"Unsupported {unit_type} unit. Supported: {list(units.keys())}"
            )
    ...
    def get_supported_units(self, unit_type: str = None) -> Dict[str, List[str]]:
        ...
        return {
            category: list(units.keys()) for category, units in UNIT_CONVERSIONS.items()
        }
