...
def search_knowledge_base(query: str, domain_weights: Dict[str, float] = None) -> str:
    ...
    try:
        ...
        payload = {"query": query, "domain_weights": domain_weights or {}, "k": 5}
...
def analyze_request(state: WorkflowState) -> WorkflowState:
    ...
    if any(
        ...
        for keyword in ["what is", "explain", "describe", "how", "why", "definition"]
    ):
    ...
    if any(
        keyword in user_message.lower()
        for keyword in ["audio", "video", "transcript", "speech", "recording"]
    ):
                        ...
                        if "Chemistry=" in line:
                            ...
                            for part in parts:
                                if "Chemistry=" in part:
                                    domain_weights["chemistry"] = float(
                                        part.split("=")[1]
                                    )
                                elif "Mechanical=" in part:
                                    domain_weights["mechanical"] = float(
                                        part.split("=")[1]
                                    )
                                elif "Materials=" in part:
                                    domain_weights["materials"] = float(
                                        part.split("=")[1]
                                    )
                except Exception:
                    pass
        ...
        if context_parts:
            ...
            if not system_found:
                enhanced_messages.insert(
                    0, {"role": "system", "content": f"Additional Context:\n{context}"}
                )

        # Use LLM backend manager for generation
        try:
            result = await llm_manager.chat_completion(
                messages=enhanced_messages, temperature=0.7, max_tokens=1000
            )
            ...
            logger.info(
                f"Successfully generated response using {llm_manager.backend_type} backend"
            )
...
async def health_check():
    ...
    try:
        ...
        try:
            ...
            basic_health["llm_backend"] = {"healthy": False, "error": str(e)}
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        logger.info(
            f"Chat request processed successfully. Tools used: {result.tools_needed}"
        )
        return response
...
async def switch_llm_backend(backend_type: str):
    ...
    try:
        if backend_type not in ["ollama", "vllm", "mock", "none", "disabled"]:
            raise HTTPException(
                status_code=400, detail="Backend must be 'ollama', 'vllm', or 'mock'"
            )
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to switch to {backend_type}"
            )
...
async def test_llm():
    ...
    try:
        test_messages = [
            {
                "role": "user",
                "content": "Hello! Please respond with 'LLM test successful' if you can read this.",
            }
        ]
        ...
        return {
            ...
            "test_response": result.get("choices", [{}])[0]
            .get("message", {})
            .get("content", ""),
            "full_result": result,
        }
        ...
        return {"success": False, "backend": llm_manager.backend_type, "error": str(e)}
