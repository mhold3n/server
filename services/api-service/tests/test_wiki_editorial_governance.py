...
def test_validate_wiki_edit_proposal_fixture() -> None:
    payload = _fixture(
        "xlotyl/services/response-control-framework/schemas/control-plane/v1/fixtures/wiki-edit-proposal/valid-minimal.json"
    )
...
def test_wiki_proposal_validation_route_accepts_valid_payload() -> None:
    ...
    assert (
        body["wiki_edit_proposal"]["caution"]
        == "unapproved source, proceed with caution"
    )
