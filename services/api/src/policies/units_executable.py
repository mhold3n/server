...
class SIUnitPolicy(BasePolicy):
        ...
        if measurements:
            ...
            if non_si_units:
                if self.enforce_si:
                    ...
                    disallowed_units = [unit for unit in non_si_units if unit not in self.allowed_units]
                    if disallowed_units:
                        violations.append(f"Disallowed units: {', '.join(disallowed_units)}")
            ...
            if consistency_issues:
                violations.append(f"Unit consistency issues: {', '.join(consistency_issues)}")
            ...
            if formatting_issues:
                violations.append(f"Unit formatting issues: {', '.join(formatting_issues)}")
                suggestions.append("Use proper unit formatting (e.g., '25°C' not '25 degrees C')")
                score -= 0.1
    ...
    def _get_si_units(self) -> dict[str, str]:
        ...
        return {
            # Length
            "meter": "m", "metre": "m", "m": "m",
            "kilometer": "km", "kilometre": "km", "km": "km",
    ...
    def _get_unit_conversions(self) -> dict[str, dict[str, float]]:
        ...
        return {
            ...
            "pressure": {
                "psi": 6894.76, "bar": 100000, "atm": 101325,
    ...
    def _find_non_si_units(self, measurements: list[dict[str, Any]]) -> list[str]:
        ...
        for measurement in measurements:
            ...
            # Check if unit is SI
            if unit not in si_units and unit not in [v.lower() for v in si_units.values()]:
    ...
    def _normalize_units(self, measurements: list[dict[str, Any]]) -> list[dict[str, Any]]:
