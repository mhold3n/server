...
def _git(cmd: list[str], cwd: Path) -> None:
    ...
    subprocess.run(["git", *cmd], cwd=str(cwd), env=env, check=True, capture_output=True)
...
def test_workspace_manager_parse_github_handles_ssh_and_https(tmp_path: Path) -> None:
    mgr = WorkspaceManager(devplane_root=tmp_path / "devplane", control_plane_root=tmp_path / "cp")
...
def test_workspace_manager_resolve_default_branch_falls_back_to_head(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_workspace_manager_detect_file_changes_and_task_packet(tmp_path: Path) -> None:
    ...
    workspace, _commands = mgr.create_workspace(project=project, branch_name="task/p1", task_id="t1")
...
def test_workspace_manager_create_workspace_returns_existing_dir_without_git(tmp_path: Path) -> None:
    ...
    workspace, commands = mgr.create_workspace(project=project, branch_name="task/p1", task_id="t2")
    assert workspace.worktree_path == str(existing_dir.resolve())
    assert commands == []


def test_workspace_manager_inspect_project_rejects_dirty_worktree(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_workspace_manager_optional_git_output_and_run_git_error_paths(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
    mgr = WorkspaceManager(devplane_root=tmp_path / "devplane", control_plane_root=tmp_path / "cp")
...
def test_workspace_manager_publish_workspace_push_and_pr_error_paths(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
    mgr = WorkspaceManager(devplane_root=tmp_path / "devplane", control_plane_root=tmp_path / "cp")
    ...
    workspace, _commands = mgr.create_workspace(project=project, branch_name="task/push", task_id="t3")
    ...
    with patch("src.devplane.workspace.shutil.which", return_value=None):
        result, commands = mgr.publish_workspace(project=project, workspace=workspace, request=request)
...
def test_workspace_manager_create_pr_success_and_failure_paths(tmp_path: Path) -> None:
    ...
    workspace, _commands = mgr.create_workspace(project=project, branch_name="task/pr", task_id="t4")
    ...
    with patch("src.devplane.workspace.shutil.which", return_value="/usr/bin/gh"):
        with patch("src.devplane.workspace.subprocess.run", return_value=fake_completed):
            pr_url, commands, errors = mgr._create_pr(workspace=workspace, project=project, request=request)
...
def test_workspace_manager_publish_workspace_no_changes_local_mode(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
    mgr = WorkspaceManager(devplane_root=tmp_path / "devplane", control_plane_root=tmp_path / "cp")
    ...
    workspace, _commands = mgr.create_workspace(project=project, branch_name="task/clean", task_id="t5")
...
def test_workspace_manager_suggest_verification_commands_and_porcelain_mapping(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
...
def test_workspace_manager_detect_file_changes_skips_short_lines(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    _init_repo(repo)
    mgr = WorkspaceManager(devplane_root=tmp_path / "devplane", control_plane_root=tmp_path / "cp")
...
async def test_execution_client_errors_and_success(tmp_path: Path) -> None:
    ...
    with patch("httpx.AsyncClient.request", new=AsyncMock(side_effect=httpx.ConnectError("nope"))):
