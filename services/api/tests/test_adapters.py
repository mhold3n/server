    ...
    with respx.mock(assert_all_called=True) as mock:
        mock.get("https://pve.local:8006/api2/json/cluster/resources").mock(
            return_value=httpx.Response(
                ...
                json={
                    "data": [
                        {"vmid": 107, "name": "docker-host", "node": "pve", "status": "stopped", "type": "qemu", "maxmem": 8589934592, "maxcpu": 4},
                        {"vmid": 104, "name": "openwrt", "node": "pve", "status": "running", "type": "qemu", "maxmem": 2147483648, "maxcpu": 2},
                    ]
                },
            )
        )
        ...
        mock.post("https://pve.local:8006/api2/json/nodes/pve/qemu/107/status/start").mock(
        ...
        # Stop VM 104
        mock.post("https://pve.local:8006/api2/json/nodes/pve/qemu/104/status/stop").mock(
            return_value=httpx.Response(200, json={"data": None})
        )
...
def test_proxmox_errors(monkeypatch):
    ...
    with respx.mock(assert_all_called=True) as mock:
        mock.get("https://pve.local:8006/api2/json/cluster/resources").mock(
            return_value=httpx.Response(200, json={"data": [{"vmid": 1, "node": "pve", "type": "qemu"}]})
        )
        mock.post("https://pve.local:8006/api2/json/nodes/pve/qemu/1/status/start").mock(
...
def test_qbittorrent_endpoints(monkeypatch):
    ...
    client = TestClient(app)
    with respx.mock(assert_all_called=True) as mock:
        mock.post("http://gluetun:8080/api/v2/auth/login").mock(return_value=httpx.Response(200, text="Ok."))
        mock.get("http://gluetun:8080/api/v2/torrents/info").mock(return_value=httpx.Response(200, json=[{"name": "test.torrent", "hash": "abc"}]))

        resp = client.get("/api/torrents/")
        assert resp.status_code == 200
        assert resp.json()["items"][0]["name"] == "test.torrent"

        mock.post("http://gluetun:8080/api/v2/torrents/add").mock(return_value=httpx.Response(200))
        resp = client.post("/api/torrents/add", json={"urls": ["magnet:?xt=urn:btih:abc"]})
        assert resp.status_code == 200 and resp.json()["added"] == 1

        mock.post("http://gluetun:8080/api/v2/torrents/pause").mock(return_value=httpx.Response(200))
        resp = client.post("/api/torrents/pause", json={"hashes": ["abc"]})
        assert resp.status_code == 200 and resp.json()["status"] == "paused"

        mock.post("http://gluetun:8080/api/v2/torrents/resume").mock(return_value=httpx.Response(200))
...
def test_qbittorrent_login_failure():
    client = TestClient(app)
    with respx.mock(assert_all_called=True) as mock:
        mock.post("http://gluetun:8080/api/v2/auth/login").mock(return_value=httpx.Response(401, text="Unauthorized"))
        resp = client.get("/api/torrents/")
        assert resp.status_code == 502
...
def test_search_endpoints(monkeypatch):
    ...
    with respx.mock(assert_all_called=True) as mock:
        mock.post("http://meilisearch:7700/indexes/files/search").mock(
            return_value=httpx.Response(200, json={"hits": [{"id": "/tmp/file.txt", "title": "file", "path": "/tmp/file.txt"}]})
        )
        mock.get("http://searxng:8080/search").mock(
            return_value=httpx.Response(200, json={"results": [{"title": "web", "url": "http://example.com"}]})
        )
...
def test_search_degraded_paths():
    ...
    with respx.mock() as mock:
        ...
        mock.post("http://meilisearch:7700/indexes/files/search").mock(return_value=httpx.Response(500))
        mock.get("http://searxng:8080/search").mock(return_value=httpx.Response(200, json={"results": []}))
        resp = client.get("/api/search", params={"q": "test", "kind": "all"})
        assert resp.status_code == 200
        assert resp.json()["files"] == []
    with respx.mock() as mock:
        # Searx fails, Meili ok
        mock.post("http://meilisearch:7700/indexes/files/search").mock(return_value=httpx.Response(200, json={"hits": []}))
