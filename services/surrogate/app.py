...
def _score_doc(query: str, doc: Dict[str, str], domain_weights: Dict[str, float]) -> float:
...
async def _apply_profile(
    ...
    if forced_fault:
        ...
        if normalized in {"500", "internal"}:
            raise HTTPException(status_code=500, detail="Surrogate forced internal failure")
...
async def rag_search(
    ...
    return {
        ...
        "reranking_method": "hybrid_bm25_embedding" if request.use_bm25_reranking else "embedding_only",
        "surrogate": profile_meta,
    }
...
def _make_transcription(seed_text: str, language: str, extract_technical: bool) -> Dict[str, Any]:
    words = re.findall(r"[A-Za-z0-9_]+", seed_text) or ["sample", "technical", "audio", "segment"]
    tokens = words[:12]
...
async def asr_transcribe_file(
    ...
    return _make_transcription(seed_text=seed_text, language=language or "en", extract_technical=extract_technical)
...
async def asr_analyze(text: str):
    text_lower = text.lower()
    matches = [k for values in TECHNICAL_KEYWORDS.values() for k in values if k in text_lower]
...
def _domain_result(domain: str, query: str, limit: int) -> Dict[str, Any]:
    templates = {
        "chemistry": [
            ("Acid-base control", "Maintain pH setpoint and confirm concentration units."),
        ...
        "materials": [
            ("Material selection", "Compare strength-to-weight trade-offs by load case."),
            ("Property validation", "Confirm yield strength and modulus from source table."),
        ],
    }
    ...
    return {
        ...
        "metadata": {
            ...
            "keywords_matched": sum(1 for k in TECHNICAL_KEYWORDS[domain] if k in query.lower()),
        },
        "query_time": 0.01,
    }
...
async def chemistry_molecular_weight(request: MolecularWeightRequest):
    ...
    for symbol, count in re.findall(r"([A-Z][a-z]?)(\d*)", request.formula):
        if symbol not in ATOMIC_WEIGHTS:
            raise HTTPException(status_code=400, detail=f"Unsupported element '{symbol}'")
...
async def mechanical_beam(request: BeamCalculationRequest):
    if request.length <= 0 or request.moment_of_inertia <= 0 or request.elastic_modulus <= 0:
        raise HTTPException(status_code=400, detail="length, moment_of_inertia, elastic_modulus must be > 0")
    moment = request.load * request.length / 4.0
    max_stress = moment * 0.05 / request.moment_of_inertia
    max_deflection = (request.load * request.length**3) / (48 * request.elastic_modulus * request.moment_of_inertia)
    safety_factor = None
    if request.material_yield_strength:
        safety_factor = request.material_yield_strength / max_stress if max_stress > 0 else None
    return {
