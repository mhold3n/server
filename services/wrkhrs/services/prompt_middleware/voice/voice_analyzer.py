...
class AdvancedVoiceAnalyzer:
        ...
        try:
            ...
            if len(audio_data) > 0:
                audio_data = librosa.resample(
                    audio_data, orig_sr=len(audio_data), target_sr=self.sample_rate
                )
    ...
    def _extract_rhythm_features(self, audio_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if len(pause_starts) > 0 and len(pause_ends) > 0:
                ...
                pause_frequency = len(pause_starts) / (
    ...
    def _extract_formant_features(self, audio_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            peaks, _ = signal.find_peaks(
                ...
                height=np.max(np.mean(magnitude, axis=1)) * 0.1,
            )
    ...
    def _extract_emotional_features(self, audio_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            valence = np.tanh(
                (np.mean(spectral_centroids) - 2000) / 1000
            )  # Normalize to [-1, 1]
    ...
    def _infer_emotional_state(self, features: VoiceFeatures) -> Dict[str, float]:
        ...
        if (
            ...
            and features.energy_mean > 0.1
            and features.arousal > 0.5
        ):
            emotions["anger"] += 0.4
        ...
        if (
            ...
            and features.arousal > 0.3
            and features.valence < -0.2
        ):
            emotions["fear"] += 0.4
    ...
    def _infer_personality_traits(self, features: VoiceFeatures) -> Dict[str, float]:
        ...
        if (
            features.energy_mean > 0.08
            and features.pitch_mean > 180
            and features.rhythm_regularity > 0.6
        ):
            personality_traits["extraversion"] = 0.4

        # Agreeableness: Moderate pitch, regular rhythm, low jitter
        if (
            160 < features.pitch_mean < 200
            and features.rhythm_regularity > 0.5
            and features.jitter < 0.06
        ):
        ...
        if (
            ...
            and features.jitter < 0.05
            and 0.06 < features.energy_mean < 0.1
        ):
            personality_traits["conscientiousness"] = 0.3

        # Neuroticism: High jitter, irregular rhythm, high pitch variance
        if (
            features.jitter > 0.08
            or features.rhythm_regularity < 0.4
            or features.pitch_std > 60
        ):
