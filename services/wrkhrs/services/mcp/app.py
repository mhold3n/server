...
class MCPService:
    ...
    def _load_domain_data(self):
        ...
        for domain_name, domain_obj in self.domains.items():
            ...
            logger.info(
                f"Loaded {domain_name} domain with {len(domain_obj.knowledge_items)} items"
            )
...
class BaseDomain:
    ...
    def __init__(self, name: str):
        ...
        self.metadata = {"name": name, "loaded_items": 0, "last_updated": None}
    ...
    def _process_text_data(self, content: str, filename: str):
        ...
        item = {"title": filename, "content": content, "source": filename}
        self._add_knowledge_item(item, filename)
    ...
    def _add_knowledge_item(self, item_data: Dict[str, Any], source: str):
        ...
        item_id = (
            item_data.get("id")
            or hashlib.md5((item_data.get("title", "") + source).encode()).hexdigest()
        )
...
class ChemistryDomain(BaseDomain):
    ...
    def calculate_ph(
        self, calculation_type: str, value: float, ion_type: str = "H+"
    ) -> Dict[str, Any]:
        ...
        try:
            if calculation_type == "concentration_to_ph":
                ...
                if ion_type == "H+":
                    ...
                    calculation_details = {"formula": "[H+] = 10^(-pH)", "pH": value}
                elif ion_type == "OH-":
                    poh = 14 - value
                ...
                raise ValueError(
                    "Invalid calculation_type. Use 'concentration_to_ph' or 'ph_to_concentration'"
                )
    ...
    def get_molecular_properties(
        self, smiles: str = None, name: str = None, formula: str = None
    ) -> Dict[str, Any]:
...
class MechanicalDomain(BaseDomain):
    ...
    def calculate_beam_stress_deflection(
        self,
        beam_type: str,
        length: float,
        load: float,
        ...
        material_yield_strength: float = None,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if beam_type == "simply_supported":
                if load_type == "point_center":
                    ...
                    max_deflection = (load * length**3) / (
                        48 * elastic_modulus * moment_of_inertia
                    )  # PL³/48EI
                ...
                if load_type == "point_end":
                    ...
                    max_deflection = (load * length**3) / (
                        3 * elastic_modulus * moment_of_inertia
                    )  # PL³/3EI
                    ...
                    max_deflection = (w * length**4) / (
                        8 * elastic_modulus * moment_of_inertia
                    )  # wL⁴/8EI
                    stress_location = "fixed end"
                    deflection_location = "free end"
                    calculations = {
                ...
                if load_type == "point_center":
                    ...
                    max_deflection = (load * length**3) / (
                        192 * elastic_modulus * moment_of_inertia
                    )  # PL³/192EI
...
class MaterialsDomain(BaseDomain):
    ...
    def lookup_material_properties(
        self, material: str, properties: List[str], temperature: float = 293.15
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for category, materials in self.materials_database.items():
                for mat_name, mat_data in materials.items():
                    if (
                        material_lower in mat_name.lower()
                        or mat_name.lower() in material_lower
                        or material_lower in category
                    ):
            ...
            for prop in properties:
                ...
                for available_prop in available_properties:
                    if (
                        prop_lower == available_prop.lower()
                        or prop_lower in available_prop.lower()
                        or available_prop.lower() in prop_lower
                    ):
        ...
        return DomainResponse(
            ...
            metadata={
                ...
                "keywords_matched": sum(
                    1
                    for kw in chemistry_domain.chemistry_keywords
    ...
    try:
        ...
        return DomainResponse(
            ...
            metadata={
                ...
                "keywords_matched": sum(
                    1
                    for kw in mechanical_domain.mechanical_keywords
                    if kw in request.query.lower()
                ),
            },
            query_time=query_time,
        ...
        raise HTTPException(
            status_code=500, detail=f"Mechanical query failed: {str(e)}"
        )
    ...
    try:
        ...
        return DomainResponse(
            ...
            metadata={
                ...
                "keywords_matched": sum(
                    1
                    for kw in materials_domain.materials_keywords
                    if kw in request.query.lower()
                ),
            },
            query_time=query_time,
...
async def upload_domain_file(
    domain_name: str, file: UploadFile = File(...), metadata: str = Form("{}")
):
...
async def calculate_ph(request: pHCalculationRequest):
    ...
    try:
        ...
        result = chemistry_domain.calculate_ph(
            request.calculation_type, request.value, request.ion_type
        )
...
async def get_molecular_properties(request: MolecularPropertyRequest):
    ...
    try:
        ...
        result = chemistry_domain.get_molecular_properties(
            smiles=request.smiles, name=request.name, formula=request.formula
        )
...
async def get_materials_database():
    ...
    for category, materials in materials_domain.materials_database.items():
        database_info[category] = {
            ...
            "sample_properties": (
                list(next(iter(materials.values())).keys()) if materials else []
            ),
        }
    ...
    return {
        ...
        "total_materials": sum(
            len(mats["materials"]) for mats in database_info.values()
        ),
    }
