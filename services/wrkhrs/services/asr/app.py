...
class ASRService:
    ...
    async def initialize(self):
        ...
        if self.use_mock:
            logger.info("USE_MOCK_MODELS=true – skipping Whisper model load (mock ASR backend active)")
            self.model = "mock"
            return
        ...
        try:
            ...
            logger.info("Whisper model loaded successfully", extra={"load_duration": self.last_load_duration})
    ...
    async def transcribe_audio(
        ...
        try:
            if self.use_mock:
                ...
                logger.info("Mock ASR backend active - returning synthetic transcription")
                dummy_text = "Mock transcription (ASR model not loaded in this environment)."
