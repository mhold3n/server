...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/logs/asr.log", mode="a"), logging.StreamHandler()],
)
...
class ASRService:
    ...
    async def initialize(self):
        """Initialize Whisper model or mock backend"""
        if self.use_mock:
            logger.info(
                "USE_MOCK_MODELS=true – skipping Whisper model load (mock ASR backend active)"
            )
            self.model = "mock"
            return
        ...
        try:
            ...
            self.last_load_duration = (datetime.utcnow() - load_start).total_seconds()

            logger.info(
                "Whisper model loaded successfully",
                extra={"load_duration": self.last_load_duration},
            )
        ...
        try:
            ...
            for chunk in response.iter_content(chunk_size=8192):
                ...
                if total_size > max_size:
                    ...
                    raise ValueError(
                        f"File too large: {total_size} bytes (max: {max_size})"
                    )
    ...
    def decode_base64_audio(self, base64_data: str, format_hint: str = "mp3") -> str:
        ...
        try:
            ...
            logger.info(
                f"Decoded {len(audio_bytes)} bytes of base64 audio to {temp_file.name}"
            )
            ...
            raise HTTPException(
                status_code=400, detail=f"Failed to decode base64 audio: {str(e)}"
            )
    ...
    def classify_segments(
        self, segments: List[Dict], threshold: float = 0.15
    ) -> List[Dict]:
    ...
    def merge_short_segments(
        self, segments: List[Dict], min_duration: float = 3.0
    ) -> List[Dict]:
    ...
    async def transcribe_audio(
        ...
        try:
            if self.use_mock:
                ...
                logger.info(
                    "Mock ASR backend active - returning synthetic transcription"
                )
                dummy_text = (
                    "Mock transcription (ASR model not loaded in this environment)."
                )
            ...
            segments, info = self.model.transcribe(
                file_path, language=language, beam_size=5, word_timestamps=True
            )
            ...
            technical_segments = (
                [seg for seg in segment_list if seg.get("is_technical", False)]
                if extract_technical
                else []
            )
...
async def health_check():
    ...
    return {
        "status": "healthy" if asr_service.model is not None else "degraded",
        "timestamp": datetime.utcnow().isoformat(),
        "model_loaded": asr_service.model is not None,
        "model_size": asr_service.model_size,
        "device": asr_service.device,
        "use_mock": asr_service.use_mock,
        "requests_total": asr_service.requests_total,
        "model_load_duration": asr_service.last_load_duration,
    }
    ...
    try:
        ...
        try:
            ...
            if file_extension in [".mp4", ".avi", ".mov", ".mkv", ".webm"]:
                ...
                success = asr_service.extract_audio_from_video(
                    temp_file_path, audio_file_path
                )
                if not success:
                    raise HTTPException(
                        status_code=400, detail="Failed to extract audio from video"
                    )
...
async def transcribe_request(request: TranscriptionRequest):
    ...
    try:
        ...
        if request.audio_url:
            ...
            temp_file_path = await asr_service.download_audio_from_url(
                request.audio_url
            )
            audio_file_path = temp_file_path
        ...
        # Transcribe the audio
        result = await asr_service.transcribe_audio(
            audio_file_path, request.language, request.extract_technical
        )
