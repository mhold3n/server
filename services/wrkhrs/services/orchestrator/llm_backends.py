...
class LLMManager:
    ...
    def _create_backend(self) -> LLMBackend:
        ...
        if self.backend_type == "ollama":
            ...
            logger.warning(
                f"Unknown backend type: {self.backend_type}, defaulting to Mock"
            )
            return MockBackend()
    ...
    async def switch_backend(self, backend_type: str) -> bool:
        ...
        try:
            ...
            if await self.backend.health_check():
                logger.info(
                    f"Successfully switched from {old_backend} to {self.backend_type}"
                )
                return True
            else:
                # Revert on failure
                ...
                self.backend = self._create_backend()
                logger.error(
                    f"Failed to switch to {backend_type}, reverted to {old_backend}"
                )
                return False
