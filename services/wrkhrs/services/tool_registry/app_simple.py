        ...
        builtin_tools = {
            ...
            "text_processor": {
                ...
                "parameters": {
                    ...
                    "operation": {
                        "type": "string",
                        "description": "Operation to perform (upper, lower, length)",
                    },
                },
                "category": "text",
                "version": "1.0.0",
                    ...
                    for name, obj in inspect.getmembers(module):
                        if inspect.isclass(obj) and hasattr(obj, "get_tool_info"):
                            try:
                                ...
                                logger.error(
                                    f"Error loading tool from {plugin_file}: {e}"
                                )
                ...
                for name, obj in inspect.getmembers(module):
                    if inspect.isclass(obj) and hasattr(obj, "get_tool_info"):
                        try:
                            tool_instance = obj()
                            tool_info = tool_instance.get_tool_info()
                            if tool_info["name"] == tool_name and hasattr(
                                tool_instance, "execute_tool"
                            ):
