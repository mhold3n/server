...
class CLIWrapperPlugin:
    ...
    def execute_cli(
        ...
        if len(args) > cmd_config["max_args"]:
            raise ValueError(
                f"Too many arguments for {command}. Max allowed: {cmd_config['max_args']}"
            )
...
if __name__ == "__main__":
    ...
    result = wrapper.execute_cli(
        "wc", ["-w"], input_data="This is a test string with words"
    )
    print(f"Result: {result}")
