...
class FileAnalysisPlugin:
    ...
    def execute_tool(
        ...
        try:
            ...
            return {"success": True, "tool": tool_name, "result": result, "error": None}
    ...
    def analyze_file(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if include_preview:
            ...
            preview = {
                ...
                "last_lines": (
                    lines[-preview_lines // 2 :] if len(lines) > preview_lines else []
                ),
    ...
    def extract_numerical_data(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if column_separator:
            ...
            data = self._extract_structured_numbers(
                content, column_separator, specific_columns
            )
        else:
        ...
        if not pattern_name and not custom_pattern:
            ...
            for name, pattern in self.predefined_patterns.items():
                matches = self._find_pattern_matches(
                    content, pattern, False, case_sensitive
                )
                if matches["count"] > 0:
    ...
    def calculate_statistics(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "outlier_percentage": len(self._detect_outliers(numbers))
                / len(numbers)
                * 100,
            },
        }
        ...
        try:
            ...
            return {
                ...
                "columns": (
                    len(headers)
                    if headers
                    else len(parsed_data[0]) if parsed_data else 0
                ),
    ...
    def validate_file_format(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            expected_format == "csv"
            or validation_results["format_detection"] == FileType.CSV
        ):
            validation_results["csv_validation"] = self._validate_csv_format(
                content, validation_rules
            )
        elif (
            expected_format == "json"
            or validation_results["format_detection"] == FileType.JSON
        ):
            validation_results["json_validation"] = self._validate_json_format(
                content, validation_rules
            )

        # Content validation
        if validation_rules:
            validation_results["content_validation"] = self._validate_content_rules(
                content, validation_rules
            )
    ...
    def _find_pattern_matches(
        self, content: str, pattern: str, include_context: bool, case_sensitive: bool
    ) -> Dict[str, Any]:
    ...
    def _detect_headers(self, first_lines: List[str]) -> bool:
        ...
        return len(re.findall(r"[a-zA-Z]", first_line)) > len(
            re.findall(r"\d", first_line)
        )
    ...
    def _assess_file_health(self, file_info: FileInfo, content: str) -> Dict[str, Any]:
        ...
        if file_info.file_type == FileType.CSV:
            lines = content.split("\n")
            consistent_cols = self._check_column_consistency(
...
if __name__ == "__main__":
    ...
    # Test file analysis
    result = analyzer.execute_tool("analyze_file", {"file_path": "/tmp/test_data.csv"})
    print(f"File analysis: {result}")

    # Test pattern finding
    result = analyzer.execute_tool(
        "find_patterns", {"file_path": "/tmp/test_data.csv", "pattern_name": "numbers"}
    )
