...
class RAGService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Could not download NLTK data, BM25 may be less effective"
                )
            ...
            if self.use_mock:
                ...
                try:
                    ...
                    logger.info(
                        f"Remote embedding dimension: {self.embedding_dimension}"
                    )
                ...
                logger.info(
                    f"Local embedding model loaded. Dimension: {self.embedding_dimension}"
                )
        ...
        try:
            ...
            try:
                ...
                if self.bm25_corpus:
                    ...
                    logger.info(
                        f"Built BM25 index with {len(self.bm25_corpus)} documents"
                    )
                else:
    ...
    def calculate_domain_score(
        self, text: str, domain_weights: Dict[str, float]
    ) -> float:
    ...
    async def add_document(
        ...
        try:
            ...
            for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                points.append(
                    PointStruct(
                        id=point_id, vector=embedding.tolist(), payload=point_metadata
                    )
                )
    ...
    async def search(
        ...
        try:
            ...
            search_results = self.qdrant_client.search(
                ...
                * 0.8,  # Lower threshold for initial retrieval
            )
            ...
            for result in search_results:
                ...
                domain_score = self.calculate_domain_score(
                    chunk_content, domain_weights or {}
                )
            ...
            if use_bm25_reranking and self.bm25_index and self.bm25_corpus:
                try:
                    ...
                    logger.debug(
                        f"Applied BM25 reranking to {len(candidates)} candidates"
                    )

                except Exception as e:
                    logger.warning(
                        f"BM25 reranking failed, falling back to embedding-only: {e}"
                    )
            ...
            final_results = [
                c for c in candidates[:k] if c["combined_score"] >= threshold
            ]
    ...
    return {
        "status": (
            ...
            if (
                ...
                or rag_service._remote_embedder
                or rag_service.use_mock
            )
...
async def upload_document(
    file: UploadFile = File(...), domain: str = "general", metadata: str = "{}"
):
...
async def add_asr_document(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add ASR document: {str(e)}"
        )
...
async def get_document_citations(document_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get citations: {str(e)}"
        )
...
async def search_by_citation(
    ...
    try:
        ...
        if technical_only:
            filter_conditions.append(
                models.FieldCondition(
                    key="citation.is_technical", match=models.MatchValue(value=True)
                )
            )
