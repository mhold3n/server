...
def main():
    parser = argparse.ArgumentParser(
        description="Setup vLLM models for multi-GPU inference"
    )
    ...
    for i, gpu in enumerate(gpus):
        print(
            f"   GPU {i}: {gpu['total']/1024:.1f}GB total, {gpu['free']/1024:.1f}GB free"
        )
    ...
    if args.model:
        ...
        for i, (size, model_info) in enumerate(available_models.items(), 1):
            print(
                f"   {i}. {size.upper()}: {model_info['name']} ({model_info['size_gb']}GB)"
            )
