...
from services.api.src.wrkhrs.domain_classifier import DomainClassifier

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger(__name__)


def reconstruct_conversation(mapping: Dict[str, Any], current_node_id: Optional[str]) -> List[Dict[str, Any]]:
...
def process_exports(input_dir: str, output_dir: str):
    ...
    rag_keywords = ["Birtha", "Claw", "mbmh", "OpenFOAM", "DOE", "architecture", "RAG", "LLM"]
    ...
    with lora_out.open('w') as flo, rag_out.open('w') as frac, skipped_out.open('w') as fsk:
        for json_file in json_files:
            ...
            for conv in conversations:
                ...
                primary_domain = classifier.get_primary_domain(full_text, threshold=0.15)
                
                # Routing logic
                # LoRA (coding examples / debugging)
                if primary_domain in lora_target_domains and len(messages) >= 3 and not direct_rag:
                ...
                # RAG (architecture, conceptual ML, specific projects)
                    ...
                    fsk.write(f"SKIPPED_DOMAIN_{domain_label.upper()} | {conv_id} | {title}\n")
                    skipped_count += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse ChatGPT Export JSON into Training/Context datasets.")
    parser.add_argument("--input", "-i", type=str, required=True, help="Directory containing conversations-*.json files")
    parser.add_argument("--output", "-o", type=str, default="data/", help="Output directory for jsonl datasets")
