...
class LLMTester:
    ...
    def __init__(self):
        ...
        self.test_messages = [
            {
                "role": "user",
                "content": "Hello! Please respond with exactly 'Test successful' and nothing else.",
            }
        ]
    ...
    async def test_backend_models(self, backend, backend_name: str) -> List[str]:
        ...
        try:
            ...
            if models:
                self.print_status(
                    f"✅ {backend_name} found {len(models)} models", "success"
                )
        ...
        try:
            ...
            if result and "choices" in result:
                ...
                if choices:
                    ...
                    self.print_status(
                        f"✅ {backend_name} chat completion successful", "success"
                    )
                    ...
                    self.print_status(
                        f"❌ {backend_name} returned empty choices", "error"
                    )
    ...
    async def test_ollama_backend(self) -> Dict[str, Any]:
        ...
        for url in urls_to_try:
            ...
            try:
                ...
                if models:
                    chat_result = await self.test_backend_chat(
                        backend, f"Ollama ({url})"
                    )
                    results["chat"] = chat_result
                else:
                    self.print_status(
                        "⚠️  Skipping chat test - no models available", "warning"
                    )
    ...
    async def test_llm_manager(self) -> Dict[str, Any]:
        ...
        try:
            ...
            manager = LLMManager()
            self.print_status(
                f"📊 LLM Manager initialized with backend: {manager.backend_type}",
                "info",
            )
            ...
            if health_info.get("healthy", False):
                ...
                self.print_status(
                    f"❌ LLM Manager health check failed: {health_info.get('error', 'Unknown')}",
                    "error",
                )
            ...
            if switch_success:
                self.print_status(
                    f"✅ Successfully switched to {target_backend}", "success"
                )
                ...
                if switch_back:
                    self.print_status(
                        f"✅ Successfully switched back to {current_backend}", "success"
                    )
                    results["switch_test"] = {"success": True}
                else:
                    self.print_status(
                        f"❌ Failed to switch back to {current_backend}", "error"
                    )
                    results["switch_test"] = {
                        "success": False,
                        "error": "Failed to switch back",
                    }
            else:
                self.print_status(f"❌ Failed to switch to {target_backend}", "error")
                results["switch_test"] = {
                    "success": False,
                    "error": f"Failed to switch to {target_backend}",
                }
    ...
    async def test_orchestrator_endpoints(self) -> Dict[str, Any]:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        if llm_response.status_code == 200:
                            ...
                            results["endpoints"]["llm_info"] = {
                                "success": True,
                                "data": llm_data,
                            }

                            backend_type = llm_data.get("backend_info", {}).get(
                                "type", "unknown"
                            )
                            is_healthy = llm_data.get("health", {}).get(
                                "healthy", False
                            )
                            ...
                            self.print_status(
                                f"❌ LLM info endpoint failed: {llm_response.status_code}",
                                "error",
                            )
                            results["endpoints"]["llm_info"] = {
                                "success": False,
                    ...
                    try:
                        ...
                        if test_response.status_code == 200:
                            ...
                            if test_data.get("success", False):
                                self.print_status(
                                    "✅ LLM test endpoint working", "success"
                                )
                                response_text = test_data.get("test_response", "")
                                print(f"   Test response: '{response_text}'")
                                results["endpoints"]["llm_test"] = {
                                    "success": True,
                                    "data": test_data,
                                }
                            else:
                                self.print_status(
                                    f"❌ LLM test failed: {test_data.get('error', 'Unknown')}",
                                    "error",
                                )
                                results["endpoints"]["llm_test"] = {
                                    "success": False,
                            ...
                            self.print_status(
                                f"❌ LLM test endpoint failed: {test_response.status_code}",
                                "error",
                            )
                            results["endpoints"]["llm_test"] = {
                                "success": False,
        ...
        if not results["tested"]:
            ...
            self.print_status(
                "💡 Make sure the orchestrator service is running", "info"
            )
    ...
    def print_summary(self):
        ...
        for test_name, result in self.test_results.items():
            ...
            if result.get("tested", False):
                if test_name == "ollama":
                    success = result.get("healthy", False) and result.get(
                        "chat", {}
                    ).get("success", False)
                elif test_name == "vllm":
                    success = result.get("healthy", False) and result.get(
                    ...
                    success = result.get("tested", False) and result.get(
                        "endpoints", {}
                    ).get("llm_test", {}).get("success", False)
                else:
                    success = True
        ...
        if passed_tests == total_tests:
            self.print_status(
                "🎉 ALL TESTS PASSED! LLM integration is working correctly.", "success"
            )
        elif passed_tests > 0:
            self.print_status(
                f"⚠️  PARTIAL SUCCESS: {passed_tests}/{total_tests} backends working.",
                "warning",
            )
        else:
            self.print_status(
                "❌ ALL TESTS FAILED: LLM integration needs attention.", "error"
            )
