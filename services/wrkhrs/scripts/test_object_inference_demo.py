...
class ObjectInferenceDemo:
    ...
    def run_demo(self):
        ...
        print("=" * 50)
        print(
            "Testing AI's ability to guess the 'object' (true intent) from ambiguous prompts"
        )
        print()
    ...
    def _simulate_middleware_processing(
        self, context: PromptContext, transformations: List
    ) -> Dict:
    ...
    def _test_voice_context_inference(self):
        ...
        for context_name, voice_features in voice_contexts.items():
            ...
            print(
                f"  Emotional State: {max(subtext.emotional_state, key=subtext.emotional_state.get)}"
            )
    ...
    def _infer_object_from_voice(self, subtext) -> str:
        ...
        if (
            ...
            subtext.urgency_level < 0.4 and subtext.emotional_state.get("fear", 0) > 0.2
        ):
            return "code_review"
        elif (
            subtext.urgency_level > 0.6
    ...
    def _test_geometric_transformations(self):
        ...
        transformations = {
            "Hyperbolic": transformer.apply_hyperbolic_transformation(
                embeddings, config
            ),
        ...
        for name, transformed in transformations.items():
            ...
            similarity = 1 - np.linalg.norm(embeddings - transformed) / np.linalg.norm(
                embeddings
            )
        ...
        multi_modal_results = transformer.apply_multi_modal_transformation(
            embeddings, config
        )
        ...
        ensemble_similarity = 1 - np.linalg.norm(
            embeddings - ensemble
