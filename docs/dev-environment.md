...
**OpenClaw workspace config** (enable `birtha-bridge`, set `birthaApiBaseUrl`) remains **outside git** unless you explicitly opt into a local JSON merge: `E2E_OPENCLAW_CONFIG_PATCH=1` **and** `--i-accept-local-config-merge` **and** `OPENCLAW_CONFIG_JSON` pointing at a file **`jq`** can rewrite. Otherwise the script prints a small JSON snippet matching the **birtha-bridge** extension under [`openclaw/extensions/birtha-bridge/`](https://github.com/XLOTYL/openclaw/tree/main/extensions/birtha-bridge) (via the **xlotyl** repo’s `openclaw` submodule).
