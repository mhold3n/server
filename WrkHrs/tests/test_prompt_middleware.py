...
try:
    ...
    pytest.skip(
        "WrkHrs prompt_middleware tests require torch (install wrkhrs[ml])", allow_module_level=True
    )
...
try:
    ...
    pytest.skip(
        "WrkHrs prompt_middleware tests require librosa (install wrkhrs[audio])",
...
class TestTransformationValidation:
    ...
    def test_mirror_transformation_preserves_meaning(self):
        ...
        similarity = 1 - np.linalg.norm(embeddings - mirrored_embeddings) / np.linalg.norm(
            embeddings
        )
