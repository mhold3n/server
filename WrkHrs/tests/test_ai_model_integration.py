...
try:
    ...
    pytest.skip(
        "WrkHrs AI model integration tests require torch (install wrkhrs[ml])",
...
try:
    ...
    pytest.skip(
        "WrkHrs AI model integration tests require librosa (install wrkhrs[audio])",
        allow_module_level=True,
    )
...
class TestAIModelIntegration:
    ...
    def _analyze_ai_response(self, response: str) -> str:
        ...
        if any(
            word in response_lower for word in ["debug", "bug", "problem", "issue", "error", "fix"]
        ):
            return "bug_in_code"
        elif any(
            ...
            word in response_lower for word in ["timeout", "slow", "performance", "bottlenecks"]
        ):
            return "timeout_issue"
        elif any(
            word in response_lower for word in ["investigate", "trace", "suspicious", "unexpected"]
        ):
    ...
    def test_middleware_ai_integration_workflow(self):
        ...
        voice_context = {"pitch_mean": 220.0, "energy_mean": 0.12, "valence": -0.4, "arousal": 0.8}
