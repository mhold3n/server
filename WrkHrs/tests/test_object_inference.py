...
try:
    ...
    pytest.skip(
        "WrkHrs object inference tests require torch (install wrkhrs[ml])", allow_module_level=True
    )
...
try:
    ...
    pytest.skip(
        "WrkHrs object inference tests require librosa (install wrkhrs[audio])",
...
class TestObjectInference:
    ...
    def _infer_object_from_results(self, results: List, scenario: Dict) -> str:
        ...
        if total_emotional_score > 0:
            all_emotional_undertones = {
                k: v / total_emotional_score for k, v in all_emotional_undertones.items()
            }
    ...
    def _get_text_inference(self, scenario: Dict) -> str:
        ...
        expected_actions = [
            action.lower() for action in scenario["context_clues"]["expected_actions"]
        ]
...
class TestAIIntegration:
    ...
    def _analyze_ai_response(self, response: str) -> str:
        ...
        if any(word in response_lower for word in ["debug", "bug", "problem", "issue", "error"]):
            ...
            word in response_lower for word in ["optimize", "performance", "faster", "improve"]
        ):
            return "performance_optimization"
        elif any(
            word in response_lower for word in ["explain", "document", "clarify", "understand"]
        ):
            return "code_explanation"
            ...
            word in response_lower for word in ["investigate", "trace", "analyze", "suspicious"]
        ):
            return "suspicious_behavior"
