...
class VoiceAnalyzer:
    ...
    def extract_prosodic_features(self, audio_data: np.ndarray) -> Dict[str, float]:
        ...
        if librosa is None:
            raise RuntimeError(
                "VoiceAnalyzer requires librosa (install the optional audio dependencies)."
            )
        try:
...
class PromptMiddleware:
    ...
    async def _apply_transformation(
        ...
        if transformation_type == TransformationType.MIRROR:
            ...
            transformed_embeddings = self.geometric_transformer.dimensional_4d_projection(
    ...
    async def _analyze_subtext(
        ...
        if context.voice_features:
            subtext["emotional_undertones"] = self.voice_analyzer.infer_emotional_context(
                context.voice_features
            )
            subtext["intentional_ambiguity"] = self.voice_analyzer.detect_intentional_ambiguity(
                context.voice_features
            )
