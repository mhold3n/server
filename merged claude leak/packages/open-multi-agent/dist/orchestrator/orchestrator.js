...
function buildAgent(config, options) {
    ...
    while (true) {
        ...
        if (ctx.abortSignal?.aborted) {
            ...
            for (const t of queue.getByStatus('pending')) {
                queue.update(t.id, { status: 'skipped' });
            }
            break;
        }
        ...
        const dispatchPromises = pending.map(async (task) => {
            ...
            const traceOptions = config.onTrace
            ...
            let retryCount = 0;
            const result = await executeWithRetry(() => pool.run(assignee, prompt, traceOptions), task, (retryData) => {
                retryCount++;
                config.onProgress?.({
                    type: 'task_retry',
                    task: task.id,
...
export class OpenMultiAgent {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            enableBuiltInTools: config.enableBuiltInTools !== false,
    ...
    agentOptions() {
        return {
            includeBuiltInTools: this.config.enableBuiltInTools,
    ...
    async runAgent(config, prompt, options) {
        ...
        const agent = buildAgent(effective, this.agentOptions());
    ...
    async runTeam(team, goal, options) {
        ...
        const coordinatorAgent = buildAgent(coordinatorConfig, this.agentOptions());
    ...
    buildPool(agentConfigs) {
        ...
        for (const config of agentConfigs) {
            ...
            pool.add(buildAgent(effective, this.agentOptions()));
        }
        return pool;
    }
    /**
